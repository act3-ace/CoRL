...
def test_mutate_observations():
    ...
    for _obs_name, obs_value in mutated_observations['red0'].items():
...
def test_filter_observations():
    ...
    for _agent_id, obs_samples in filtered_observations.items():
        for _obs_name, obs_value in obs_samples.items():
