...
class Vector(NonTerminalMetric):
    ...
    _arr: typing.List[Metric]
    _is_time_series: bool

    def __init__(self, arr: typing.List[Metric]):
        self._arr = arr

        # Not sure what a "generator" is
        self._is_time_series = all([isinstance(item, TimedValue) for item in self._arr])  # pylint: disable=use-a-generator

    @property
    def arr(self) -> typing.List[Metric]:
