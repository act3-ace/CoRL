...
def create_gif(image_directory: str, output_filename: str, fps: int):
    ...
    with imageio.v2.get_writer(output_filename, fps=fps) as writer:
...
def main(pickle_files: typing.List, parse_args: jsonargparse.ArgumentParser):
    ...
    for file in tqdm(pickle_files):
        ...
        if os.path.isdir(parse_args.input):
            file_to_display = str(file.name).replace(parse_args.input, '...')
        else:
            file_to_display = parse_args.input
        text_block = []
...
if __name__ == '__main__':
    ...
    if os.path.isdir(args.input):
        ...
        episode_files = file_df['full_filepath'].tolist()  # pylint: disable=unsubscriptable-object
    else:
